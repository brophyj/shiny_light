# app_full.R -- Bayesian + frequentist re-analysis of a single RCT
#
# This is the full-feature variant of the Shiny app, sharing all
# analytic helpers with `rct_pipeline.qmd`.  If you only want the
# Bayesian outputs (no S-values, Type M/S, or Design-informed prior),
# use `app_bayes_alone.R` instead.
#
# Features (vs app_bayes_alone.R, additions marked +):
#   * Up to FIVE priors: Weak / Enthusiastic / Skeptical /
#     Empirical-Bayes (van Zwet et al. 2021/2025) /
#     + Design-informed (optional, centred on user-supplied design HR).
#   * Three likelihoods: Normal (analytic), Binomial (Laplace), MCMC
#     (brms + cmdstanr) on demand.
#   * MCID-aware posterior tables and plots, including
#     "Prior vs Likelihood vs Posterior" facet and a compact forest tab.
#   * + Frequentist calibration block (sidebar/main panel):
#       Greenland S-values vs RR=1 and vs RR=MCID, plus
#       Gelman-Carlin Type-S / Type-M / design SE retrodesign metrics.
#   * + Calibration tab:
#       per-prior posterior calibration metrics (Type-S, Type-M median,
#       Coverage, Replication) computed from posterior SNR draws, plus
#       the EB mixture component decomposition (B_k, B_bar).
#
...
# ---------- source helpers ----------
# `pipeline.R` is included alongside the Shiny-only helpers so the app
# can reuse the same Shannon S-value and Gelman-Carlin Type M/S
# implementations as `rct_pipeline.qmd`.  Only the small calibration
# helpers (s_value, type_m_s) and the build_priors / Design-informed
# prior support are used from it; the heavy brms wrappers are still only
...
local({
  ...
  for (f in c("priors.R", "likelihoods.R", "eb_mixture.R",
              "summaries.R", "plots.R", "pipeline.R")) {
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        condition = "input.effect_measure == 'RR'",
        hr(),
        h4("Design-informed prior (optional)"),
        helpText(
          "Adds a 5th prior centred on the trial's pre-specified effect on ",
          "the log-RR scale. Leave ", tags$code("design_hr"),
          " blank to omit this prior."
        ),
        numericInput("design_hr", "Design HR (RR scale)", value = NA,
                     min = 1e-3, step = 0.01)
      ),
      hr(),
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Observed / Summary",
                 ...
                 h5("Frequentist calibration"),
                 htmlOutput("calibration_block"),
        ...
        tabPanel("Calibration",
                 helpText(em("Calibration metrics are computed on the RR scale.")),
                 h5("Per-prior posterior calibration metrics"),
                 helpText(
                   "SNR = log(RR) / SE. Type-S = P(opposite sign), ",
                   "Type-M (median) = median |z / SNR_post|, ",
                   "Coverage = posterior mass within +/- 1.96 of the ",
                   "observed z, Replication = same-sign re-significance ",
                   "probability."),
                 tableOutput("calib_tbl"),
                 h5("Empirical-Bayes mixture decomposition"),
                 helpText(
                   "Posterior weights of the four van Zwet components ",
                   "after seeing the observed z. ",
                   tags$code("B_k"),
                   " is the per-component shrinkage factor; ",
                   tags$code("B_bar"), " is the pooled posterior shrinkage."),
                 tableOutput("eb_components_tbl"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # Main-panel observed block — always shows both RR and RD.
  ...
  # Mirrors the per-study `derived-quantities` chunk in rct_pipeline.qmd:
  # we compute Greenland S-values vs the null (RR = 1) and vs the active
  # MCID, plus the Gelman-Carlin Type-S / Type-M errors when the trial's
  # power is supplied.
  calibration_parts <- reactive({
    o <- obs(); f <- obs_freq_parts(); m <- mcid()
    out <- list(s_null = NA_real_, s_mcid = NA_real_,
                type_s = NA_real_, type_m = NA_real_, design_se = NA_real_)
    if (is.finite(f$p)) {
      out$s_null <- s_value(pmax(f$p, .Machine$double.eps))
    }
    if (is.finite(o$y) && is.finite(o$sigma) && is.finite(m) && m > 0) {
      z_mcid <- (o$y - log(m)) / o$sigma
      p_mcid <- 2 * pnorm(-abs(z_mcid))
      out$s_mcid <- s_value(pmax(p_mcid, .Machine$double.eps))
    }
    pw <- input$pow_target
    if (isTruthy(pw) && is.finite(pw) && pw > 0 && pw < 1) {
      tms <- tryCatch(type_m_s(power = pw, target_effect = 1, n_sim = 50000),
                      error = function(e) NULL)
      if (!is.null(tms)) {
        out$type_s    <- tms$type_s
        out$type_m    <- tms$type_m
        out$design_se <- tms$se
      }
    }
    out
  })

  output$calibration_block <- renderUI({
    cal <- calibration_parts()
    parts <- character(0)
    if (is.finite(cal$s_null)) {
      parts <- c(parts, sprintf("<em>S</em> vs RR=1 = <strong>%.2f</strong> bits",
                                cal$s_null))
    }
    if (is.finite(cal$s_mcid)) {
      parts <- c(parts, sprintf("<em>S</em> vs RR=MCID = <strong>%.2f</strong> bits",
                                cal$s_mcid))
    }
    if (is.finite(cal$type_m)) {
      parts <- c(parts,
                 sprintf("Type-S = %.3f, Type-M (median) = %.2f%s",
                         cal$type_s, cal$type_m,
                         ifelse(is.finite(cal$design_se),
                                sprintf(" (design SE = %.2f)", cal$design_se),
                                "")))
    }
    if (!length(parts)) {
      return(HTML("<small><em>S-values and Type M/S require event counts and a target power.</em></small>"))
    }
    HTML(paste0("<small>", paste(parts, collapse = "<br>"), "</small>"))
  })
  ...
  # Helper: pipeline-style Design-informed prior on log-RR.
  # `mu0 = log(design_hr)`, `tau = SE_logRR_observed`.  Returns NULL when
  # the user has not entered a positive `design_hr`, so callers can decide
  # whether to include the 5th prior slot.
  design_prior_logRR <- function() {
    dh <- input$design_hr
    if (!isTruthy(dh) || !is.finite(dh) || dh <= 0) return(NULL)
    o  <- obs()
  ...
  all_priors_logRR <- reactive({
    ...
    di <- design_prior_logRR()
    if (!is.null(di)) out$`Design-informed` <- di
    out
  })
  ...
  posterior_df <- reactive({
    ...
    if (identical(like, "normal")) {
      ...
      di <- design_prior_logRR()
      if (!is.null(di)) pars$`Design-informed` <- di
      ...
      isolate({
        ...
        di <- design_prior_logRR()
        if (!is.null(di)) priors$`Design-informed` <- di
  ...
  # ----- per-prior posterior calibration table (matches rct_pipeline.qmd) -
  calib_summary_tbl <- reactive({
    df <- posterior_df(); o <- obs()
    if (!is.finite(o$sigma) || o$sigma <= 0 || !is.finite(o$y)) {
      return(tibble::tibble(prior = character(0)))
    }
    z_obs <- o$y / o$sigma
    grp <- split(df, df$prior)
    rows <- lapply(names(grp), function(nm) {
      snr <- grp[[nm]]$mu / o$sigma
      snr <- snr[is.finite(snr)]
      if (!length(snr)) {
        return(tibble::tibble(prior = nm,
                              `Type-S`        = NA_real_,
                              `Type-M (med.)` = NA_real_,
                              Coverage        = NA_real_,
                              Replication     = NA_real_))
      }
      type_s <- mean(sign(z_obs) != sign(snr))
      type_m <- stats::median(abs(z_obs / snr))
      cover  <- mean(snr > z_obs - 1.96 & snr < z_obs + 1.96)
      repl   <- {
        z2 <- snr + rnorm(length(snr))
        mean(abs(z2) > 1.96 & sign(z2) == sign(z_obs))
      }
      tibble::tibble(prior = nm,
                     `Type-S`        = type_s,
                     `Type-M (med.)` = type_m,
                     Coverage        = cover,
                     Replication     = repl)
  ...
  output$calib_tbl <- renderTable(calib_summary_tbl(),
                                  digits = 3, striped = TRUE, hover = TRUE)

  # ----- EB mixture component decomposition table -----------------------
  # Renders the per-component posterior weight, conditional mean (B_k * z),
  # SD (sqrt(B_k)), and shrinkage factor B_k after seeing the observed z,
  # plus the pooled shrinkage factor B_bar.  Mirrors the eb-components
  # chunk in rct_pipeline.qmd.
  eb_components_tbl <- reactive({
    o <- obs()
    if (!is.finite(o$y) || !is.finite(o$sigma) || o$sigma <= 0) {
      return(tibble::tibble(component = integer(0)))
    }
    z   <- o$y / o$sigma
    fit <- eb_posterior_snr(z)
    comp <- fit$components
    tibble::tibble(
      component = seq_len(nrow(comp)),
      weight    = comp$weight,
      mean      = comp$mean,
      sd        = comp$sd,
      B_k       = comp$B,
  ...
  output$eb_components_tbl <- renderTable(eb_components_tbl(),
                                          digits = 3, striped = TRUE, hover = TRUE)
      ...
      if (is_rd) {
        ...
        header <- c(
          ...
          sprintf("# Counts (t / c)    : %d / %d events out of %d / %d",
                  input$e_t, input$e_c, input$n_t, input$n_c),
          sprintf("# S vs RR=1         : %s",
                  ifelse(is.finite(cal$s_null), sprintf("%.2f bits", cal$s_null), "NA")),
          sprintf("# S vs RR=MCID      : %s",
                  ifelse(is.finite(cal$s_mcid), sprintf("%.2f bits", cal$s_mcid), "NA")),
          sprintf("# Type-S / Type-M   : %s / %s",
                  ifelse(is.finite(cal$type_s), sprintf("%.3f", cal$type_s), "NA"),
                  ifelse(is.finite(cal$type_m), sprintf("%.2f", cal$type_m), "NA")),
          sprintf("# Design HR (prior) : %s",
                  ifelse(isTruthy(input$design_hr) &&
                           is.finite(input$design_hr) && input$design_hr > 0,
                         sprintf("%.3f", input$design_hr), "not used")),
          "#"
        )
      }
